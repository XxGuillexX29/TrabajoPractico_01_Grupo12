...
import "./Css/PPT.css";
import sonido02 from "./Sonidos/sonidoGanar.mp3"
import sonido04 from "./Sonidos/sonidoClick.mp3";
    ...
    <div className="App">
      ...
      <div className='eleccion'>
        <div className='Jugador'>
          <img className='manoComputadora' src={`img/${eleccionJugador}.png`} alt=''></img>
        </div>
        <div className='eleccion-computadora'>
          <img className='manoComputadora' src={`img/${eleccionComputadora}.png`} alt=''></img>
        </div>
      </div>

      <div className='botonPPT'>
        {elecciones.map((eleccion, index) =>
          <button className='boton' key={index} onClick={() => hacerClick(eleccion) } disabled={gameOver}>
            {eleccion}
          </button>
        )}
      </div>
      ...
      <div className='botonReiniciar'>
        {gameOver &&
          <button className='boton' onClick={() => reiniciar()}>¿Reiniciar Juego?</button>
        }
      </div>
    </div>
  )
